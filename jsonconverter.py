...
def load_text_file(file_name):
    try:
        ...
        print(f"⚠️ Error loading text file '{file_name}': {e}")
        return ""
...
#     except Exception as e:
#         print(f"⚠️ Error writing chat to file: {e}")
...
def add_to_file():
    ...
    if not text_to_add:
        return jsonify({"message": "⚠️ No text provided to add."}), 400

    success = append_to_file(text_to_add)
    if success:
        return jsonify({"message": "✅ Text successfully added to the file."})
    else:
        return jsonify({"message": "⚠️ Failed to add text to the file."}), 500
